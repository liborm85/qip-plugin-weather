...
*
*  revision history
*    2006.04.26  updated to version 1.2.6
*                J. Rathlev (rathlev(a)physik.uni-kiel.de)
*                changes:  TZDecompressionStream.Create -
*                          number of bytes to decompress added
*    2006.03.14  updated to version 1.2.5
*                J. Rathlev (rathlev(a)physik.uni-kiel.de)
*                changes:  TZCompressionStream.Write
*                          uses Write instead of WriteBuffer                                                           *
*    2005.11.10  updated to version 1.2.4
*                J. Rathlev (rathlev(a)physik.uni-kiel.de)
*                fixed:    FStreamPos must be of type "int64"
*                changes:  all constants reside in interface part
*                          crc32 and adler32 included
...
begin
  ...
  try
    ...
    while ZCompressCheck(deflate(FZStream,Z_FINISH)) <> Z_STREAM_END do begin
      ...
      n:=FStream.Write(FBuffer,SizeOf(FBuffer) - FZStream.avail_out); // changed to "write" to suppress exception

      FZStream.next_out := FBuffer;
      FZStream.avail_out := SizeOf(FBuffer);
    end;

    if FZStream.avail_out < SizeOf(FBuffer) then begin
      n:=FStream.Write(FBuffer,SizeOf(FBuffer) - FZStream.avail_out); // changed to "write" to suppress exception
    end;
  finally
    deflateEnd(FZStream);
...
begin
  ...
  while FZStream.avail_in > 0 do begin
    ...
    if FZStream.avail_out = 0 then begin
      n:=FStream.Write(FBuffer,SizeOf(FBuffer)); // changed to "write" to suppress exception
      if n<SizeOf(FBuffer) then begin
        result:=n; exit;
        end;
...
begin
  ...
  while (FZStream.avail_out > 0) and (zresult <> Z_STREAM_END) do begin
    if FZStream.avail_in = 0 then begin
      if SLength>SizeOf(FBuffer) then n:=SizeOf(FBuffer) else n:=SLength;
      dec(SLength,n);
